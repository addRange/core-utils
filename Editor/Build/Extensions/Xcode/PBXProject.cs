    ...
    public class PBXProject
    {
        ...
        */
        ...
        internal void SetTargetAttributes(string key, string value)
        {
		 ...
		 public string[] CreateGroups(string sourceGroup, string[] relPathesForGroups)
		 {
			 var fixedSource = PBXPath.FixSlashes(sourceGroup);
			 string sourceGroupName = Path.GetFileName(sourceGroup);
			
			 string[] addedGuids = new string[relPathesForGroups.Length];
			 for (var i = 0; i < relPathesForGroups.Length; i++)
			 {
				 var relPathForGroup = relPathesForGroups[i];
				 string fileName = Path.GetFileName(relPathForGroup);
				 string fileGuid = AddFileImpl(fileName, relPathForGroup, PBXSourceTree.Group, false);
				 addedGuids[i] = fileGuid;
			 }


			 var targetGroup = GroupsGetByName(sourceGroupName);
			 if (fixedSource.StartsWith("/"))
			 {
				 fixedSource = fixedSource.Substring(1);
			 }
			 targetGroup.path = fixedSource;
			 targetGroup.name = sourceGroupName;
			 targetGroup.UpdateProps();
			 //mainRelativeGroup.UpdateProps();
			 return addedGuids;
		 }

		 //private string FindBestExistingPath(List<string> addedGroups, string relPathesForGroup)
		 //{
			// string best = null;
			// int maxLength = int.MinValue;
			// foreach (var addedGroup in addedGroups)
			// {
			//	 if (!relPathesForGroup.StartsWith(addedGroup))
			//	 {
			//		 continue;
			//	 }
			//	 int addedGroupLength = addedGroup.Length;
			//	 if (maxLength > addedGroupLength)
			//	 {
			//		 continue;
			//	 }
			//	 best = addedGroup;
			//	 maxLength = addedGroupLength;
			// }
